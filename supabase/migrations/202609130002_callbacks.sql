create table public.provider_events(id bigint generated always as identity primary key,message_id uuid not null references notification_messages(id),provider_sid text not null,state text not null,error text,received_at timestamptz not null default now(),unique(message_id,provider_sid,state));
alter table public.provider_events enable row level security;
revoke all on public.provider_events from anon,authenticated;

create function public.record_whatsapp_status(message_id uuid,sid text,new_state text,provider_error text) returns void language plpgsql security definer set search_path=public,pg_temp as $$
...
begin
 ...
 if sid is null or sid !~ '^SM[0-9a-fA-F]{32}$' then raise exception 'Invalid provider message ID'; end if;
 -- Twilio's queued means accepted by Twilio, never queued for our worker to send again.
