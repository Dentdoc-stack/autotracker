-- Suppress individual event occurrences across overlapping digests.
drop function public.digest_candidates(date,timestamptz);
...
create or replace function public.schedule_daily_digest() returns integer language plpgsql security definer set search_path=public,pg_temp as $$
...
create or replace function public.claim_reminder_batch() returns setof public.notification_messages language plpgsql security definer set search_path=public,pg_temp as $$
begin
 ...
 where n.state='queued' and (
   ...
   or not exists(select 1 from public.digest_candidates(n.due_date,null,n.id) c where c.contact_id=n.contact_id and c.contact_version=n.contact_version and c.phone=n.phone_snapshot and c.body=n.body and c.item_keys=n.item_keys));
